...
public class ValidateID {
    ...
    public static long validateID(String id_feed) throws InputWasNullException, InputNotAnIntegerException, ValueOutOfRangeException {
        if (id_feed == null) {
            Complain("ID was null");
            throw new InputWasNullException("ID was null");
        }
        long id;
        try {
            ...
            Complain(t);
            throw new InputNotAnIntegerException(id_feed + " could not be parsed as an integer.");
        }

        if (id_feed.length() > maxLen || (minId != null && id < minId) || (maxId != null && maxId < id)) {
            throw new ValueOutOfRangeException ("ID "+id+" was not in the service-enforced range constraint");
        }
        return id;
    }
}
